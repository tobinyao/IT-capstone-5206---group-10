...
def slope_to_risk(slope_value, max_slope_degrees=45):
    """Convert slope degrees to Slope Risk on a 0-100 scale."""
    if slope_value is None:
        return 0
    return clamp_score((float(slope_value) / max_slope_degrees) * 100)


def fuel_to_risk(fuel_value, max_fuel_age_years=30, fuel_type_mapping=None):
    """
    Convert fuel age or fuel type to Fuel Risk.

    Numeric values are treated as fuel age in years. String values are treated
    as fuel type categories.
    """
    if fuel_value is None:
        return 0

    if isinstance(fuel_value, (int, float)):
        return clamp_score((fuel_value / max_fuel_age_years) * 100)
...
def burn_context_to_risk(burn_context, burn_context_mapping=None):
    ...
    values are treated as overlap / no overlap.
    """
    if burn_context is None:
        return 0

    if isinstance(burn_context, bool):
        return 80 if burn_context else 10
...
def standardize_risk_inputs(data, config=None):
    ...
    fuel_value = data.get("fuelAge", data.get("fuelType"))
